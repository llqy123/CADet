...
class ZeroShotClassifier(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        with open('cadetdata/coco_categories_info.json', 'r') as f:
        ...
        visual_prototype = torch.from_numpy(np.load('cadetdata/visual_prototype_v2.npy'))
    ...
    def forward(self, input_x, ann_type='box', classifier=None):
        ...
        proj_x = F.normalize(proj_x, p=2, dim=1)
        scores2 = proj_x @ self.visual_prototype.T
        ...
        x = torch.mm(proj_x, zs_weight) # [860, 65]

        if not self.training:
            x[:, self.unseen_cls] = self.norm_temperature * (
                        x[:, self.unseen_cls] + 0.08 * (scores2[:, self.unseen_cls]))
    ...
    def forward_baseline(self, input_x, ann_type='box', classifier=None):
        '''
        Inputs:
            x: B x D'
            classifier_info: (C', C' x D)
