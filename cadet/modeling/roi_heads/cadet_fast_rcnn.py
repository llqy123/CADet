...
from ..utils import load_class_freq, get_fed_loss_inds, load_ot_weight_mask
from .zero_shot_classifier import ZeroShotClassifier, SinkhornDistance


__all__ = ["CADetFastRCNNOutputLayers"]


class CADetFastRCNNOutputLayers(FastRCNNOutputLayers):
    @configurable
    def __init__(
        self, 
    ...
    def from_config(cls, cfg, input_shape):
        ...
        ret.update({
            ...
            'ot_loss_weight': cfg.MODEL.ROI_BOX_HEAD.OT_LOSS_WEIGHT,
    ...
    def align_sinkhorn_loss(self, predictions, proposals, targets, classifier_info=(None,None,None), ann_type='image'):
        scores, proposal_deltas, proj_region, zs_weight = predictions
        ...
        for ii, (proj_r, target) in enumerate(zip(proj_region, targets)):
            ...
            matched_gt_inds = [tag_gt_cate[i] for i in matched_gt_inds]
            target_for_ot = torch.zeros_like(similarity_for_img)
            y_s = torch.tensor(range(0,self.ws_num_props)).to(zs_weight.device)
            ...
            for xx_s, yy_s in zip(matched_gt_inds, y_s):
                ...
                for jj in range(len(targets)):
                    if jj == ii: 
                        continue
                    other_concepts = targets[jj]
                    neg_concept_list = [other_concept_id for other_concept_id in other_concepts._pos_category_ids \
                        if other_concept_id not in targets[ii]._pos_category_ids]
                    word_features[:, match_num:len(neg_concept_list)+match_num] = zs_weight[:, neg_concept_list]
                    match_num = match_num + len(neg_concept_list)
                optimize_matrix = word_features[:,:match_num].permute(1,0) @ proposal_feature
        ...
        return loss/batch_size * self.ot_loss_weight

    def align_contrastive_loss(self, predictions, proposals, targets, classifier_info=(None,None,None), ann_type='image'):
        scores, proposal_deltas, proj_region, zs_weight = predictions
        batch_size = len(proposals)
        num_inst_per_image = [len(p) for p in proposals]
        # print(num_inst_per_image)
        proj_region = proj_region.split(num_inst_per_image, dim=0)     
        loss = proj_region[0].new_zeros([1])[0] #(proj_region[0][:,0]).repeat((zs_weight.size()[1]-1,1))
        ...
        for ii, (proj_r, target) in enumerate(zip(proj_region, targets)):
            ...
            similarity_for_img = torch.mm(zs_weight.permute(1,0), proj_r)
            ss = similarity_for_img[:-1,:]
            norm_similarity_for_img = ss-ss.min()
            ot_similarity = torch.zeros(int(len(target_ids)), proj_r.size()[1]).to(zs_weight.device)
            for i, ind in enumerate(target_ids):
                ot_similarity[i] = norm_similarity_for_img[ind]
            distance_for_ot = -ot_similarity
            x_s, y_s = linear_sum_assignment(distance_for_ot.detach().cpu())
            x_s = [target_ids[i] for i in x_s]
            ...
            for xx_s, yy_s in zip(x_s, y_s):
                ...
                target_matrix[0] = 1
                loss += torch.sum(F.binary_cross_entropy_with_logits(optimize_matrix, target_matrix, reduction='none'))
                similarities = F.cosine_similarity(0.8 * proposal_feature.unsqueeze(0) + 0.2 * zs_weight[:, xx_s].unsqueeze(0), proj_r.permute(1, 0), dim=1)
                similarities[yy_s] = -float('inf')
                topkk = similarities.topk(5).indices
                for yy_s in topkk:
                    if similarities[yy_s] > 0.95 and yy_s not in y_s:
                        match_num = 0
                        word_features = torch.zeros_like(zs_weight)  # [1024, 4765]
                        proposal_feature = proj_r[:, yy_s]  # [1024] 给定候选框特征
                        word_features[:, match_num] = zs_weight[:, xx_s]  # [1024, 4765]
                        match_num = match_num + 1
                        word_features[:, match_num:len(neg_concept_list) + match_num] = zs_weight[:, neg_concept_list]
                        match_num = match_num + len(neg_concept_list)
                        optimize_matrix = word_features[:, :match_num].permute(1, 0) @ proposal_feature
                        ...
                        target_matrix[0] = 1
                        loss += torch.sum(
                            F.binary_cross_entropy_with_logits(optimize_matrix, target_matrix, reduction='none'))
        return loss / batch_size * self.ot_loss_weight


    def align_BCE_loss(self, predictions, proposals, targets, classifier_info=(None,None,None), ann_type='image'):
        scores, proposal_deltas, proj_region, zs_weight = predictions
        batch_size = len(proposals)
        num_inst_per_image = [len(p) for p in proposals]
        proj_region = proj_region.split(num_inst_per_image, dim=0)        
        loss = torch.zeros_like(proj_region[0][:-1,0]).repeat((zs_weight.size()[1]-1,1))
        for ii, (proj_r, target) in enumerate(zip(proj_region, targets)):            
            proj_r = proj_r[:-1,:].permute(1,0)
            similarity_for_img = torch.mm(zs_weight.permute(1,0), proj_r)
            ss = similarity_for_img[:-1,:]
            norm_similarity_for_img = ss-ss.min()
            ot_similarity = torch.zeros(int(len(target._pos_category_ids)), proj_r.size()[1]).to(zs_weight.device)
            for i, ind in enumerate(target._pos_category_ids):
                ot_similarity[i] = norm_similarity_for_img[ind]
            distance_for_ot = -ot_similarity
            x_s, y_s = linear_sum_assignment(distance_for_ot.detach().cpu())
            x_s = [target._pos_category_ids[i] for i in x_s]#[np.ndarray.tolist(x_s)]

            target = torch.zeros_like(similarity_for_img)
            target[x_s,y_s] = 1.
            loss += F.binary_cross_entropy_with_logits(similarity_for_img[:-1], target[:-1], reduction='none')

        return torch.sum(loss)/batch_size * self.ot_loss_weight
    ...
    def image_label_losses(self, predictions, proposals, image_labels, \
        ...
        for idx, (score, labels, prop_score, p) in enumerate(zip(
            ...
            neg_concept_list = []
            for neg_label in image_labels[:idx]+image_labels[idx+1:]:
                other_concepts = neg_label
                neg_concept_list += [other_concept_id for other_concept_id in other_concepts \
                    if other_concept_id not in labels]
            ...
            for i_l, label in enumerate(labels):
                ...
                if self.image_label_loss in ['wsod', 'wsddn']: 
                    ...
                    loss_i, ind = self._max_size_loss(score, label, neg_concept_list, p)
    ...
    def forward(self, x, ann_type='box', classifier_info=(None,None,None)):
        ...
        if classifier_info[0] is not None:
            cls_scores, proj_region, zs_weight = self.cls_score(x, ann_type ,classifier=classifier_info[0])
        ...
        if classifier_info[2] is not None:
            ...
            if self.sync_caption_batch:
                caption_scores, _, _ = self.cls_score(x, classifier=cap_cls[:, :-1]) 
            else:
                caption_scores, _, _ = self.cls_score(x, classifier=cap_cls)
        ...
        if self.with_softmax_prop:
            ...
            return scores, proposal_deltas, proj_region, zs_weight
    ...
    def _max_size_loss(self, score, label, neg_concept_list, p):
        loss = 0
        target = score.new_zeros(score.shape[1])
        target[label] = 1.
        ...
        if self.softmax_weak_loss:
            ...
            loss += F.binary_cross_entropy_with_logits(
                score[ind][[label]+neg_concept_list], target[[label]+neg_concept_list], reduction='sum')  #81 category
        return loss, ind
