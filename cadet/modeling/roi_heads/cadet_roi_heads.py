...
from .cadet_fast_rcnn import CADetFastRCNNOutputLayers
...
class DeticCascadeROIHeads(CascadeROIHeads):
    ...
    def from_config(cls, cfg, input_shape):
        ...
        ret.update({
            ...
            'use_ot':cfg.MODEL.ROI_BOX_HEAD.USE_OT,
            'use_caption': cfg.MODEL.ROI_BOX_HEAD.USE_CAPTION,
        })
        return ret
        ...
        for k in range(self.num_cascade_stages):
            if k > 0:
                ...
                proposals = self._create_proposals_from_boxes(
                    ...
                    logits=[p.objectness_logits for p in proposals], ann_type=ann_type)
                ...
                with storage.name_scope("stage{}".format(stage)):
                    if ann_type != 'box': 
                        ...
                        if ann_type in ['image', 'caption', 'captiontag']:
                            ...
                            #########################
                            if self.use_caption:
                                caption_losses = predictor.image_label_losses(
                                    predictions, proposals, image_labels,
                                    classifier_info=classifier_info,
                                    ann_type=ann_type)
                                stage_losses['caption_loss'] = caption_losses
                            ##########################
                            if self.use_ot == 'BCE':
                                ot_loss = predictor.align_BCE_loss(
                                    predictions, proposals, targets,
                                    classifier_info=classifier_info,
                                    ann_type=ann_type)
                                stage_losses['ot_loss'] = ot_loss
                            elif self.use_ot == 'contrastive':
                                ot_loss = predictor.align_contrastive_loss(
                                    predictions, proposals, targets,
                                    classifier_info=classifier_info,
                                    ann_type=ann_type)
                                stage_losses['ot_loss'] = ot_loss
                            elif self.use_ot == 'sinkhorn':
                                ot_loss = predictor.align_sinkhorn_loss(
                                    predictions, proposals, targets,
                                    classifier_info=classifier_info,
                                    ann_type=ann_type)
                                stage_losses['ot_loss'] = ot_loss
                            stage_losses['loss_cls'] =  predictions[0].new_zeros([1])[0]
                            stage_losses['loss_box_reg'] = predictions[0].new_zeros([1])[0]
                        ...
                        stage_losses = predictor.losses(
                            ...
                            classifier_info=classifier_info)
                        if self.use_ot != '':
                            stage_losses['ot_loss'] = \
                                predictions[0].new_zeros([1])[0]
                        if self.use_caption:
                            stage_losses['caption_loss'] = \
                                predictions[0].new_zeros([1])[0]
    ...
    def get_top_proposals(self, proposals):
        ...
        for i in range(len(proposals)):
            ...
            ##########
            inds = proposals[i].proposal_boxes.nonempty()
            if False in inds:
                proposals[i] = proposals[i][inds]
            ##########
    ...
    def _create_proposals_from_boxes(self, boxes, image_sizes, logits, ann_type):
        """
    ...
    def _run_stage(self, features, proposals, stage, \
        ...
        if proposals == None:
            ...
            box_features = self.box_head[stage](box_features) #[[4096(8*512), 1024]]
            return box_features

        pool_boxes = [x.proposal_boxes for x in proposals]
        box_features = self.box_pooler(features, pool_boxes) #[4096(8*512), 256, 7, 7]
        box_features = _ScaleGradient.apply(box_features, 1.0 / self.num_cascade_stages)
        box_features = self.box_head[stage](box_features) #[[4096(8*512), 1024]]
